...
function WebRTCAdaptor(initialValues) {
  ...
  this.iceCandidateReceived = function (event, streamId) {
    if (event.candidate) {
      ...
      if (protocolSupported) {
        ...
        console.log(
          "Candidate's protocol(full sdp: " + event.candidate.candidate + ') is not supported. Supported protocols: ' + thiz.candidateTypes
        );
        if (event.candidate.candidate != '') {
          //
          thiz.callbackError(
            'protocol_not_supported',
            'Support protocols: ' + thiz.candidateTypes.toString() + ' candidate: ' + event.candidate.candidate
          );
        }
      }
    } else {
  ...
  this.getVideoSender = function (streamId) {
    ...
    if (
      (adapter.browserDetails.browser === 'chrome' || (adapter.browserDetails.browser === 'firefox' && adapter.browserDetails.version >= 64)) &&
